...
- **Config**: `tools/levelgen/config.js`
  ...
  - `levels`: array of “batches” describing how to generate groups of levels:
    ...
    - `gridWidth`, `gridHeight`: board cell counts (`x` in `[0, gridWidth)`, `y` in `[0, gridHeight)`; both ≥ 5). Legacy `gridSize` is shorthand for a square grid. **Both may be omitted**: the generator then infers a minimal board from the template footprint and every layer’s silhouette (see [`inferGridDimensions` in `tools/levelgen/generator.js`](tools/levelgen/generator.js)). Inference uses a large reference grid, unions layer cell bounding boxes, applies padding (`gridInferMargin`, default **0**), then widens until all layers fit. Set `gridInfer: false` to require an explicit grid. **`heart`** requires **`templateParams.radius`** (symmetric) or **both `radiusX` and `radiusY`** when the grid is omitted — the silhouette is derived only from those parameters (centered on the board), not from stretching to the board edges. **`rectangle`** needs integer `templateParams.width` and `height`. Templates that use [`resolveRadii`](tools/levelgen/templates.js) must supply either symmetric **`radius`** or **both `radiusX` and `radiusY`** so axes are not defaulted from grid size. `generateOneLevel` / CLI pass `inferCtx` `{ seed, batchIndex, slotIndex }` when the grid is omitted (matches batch-variation slot keys). **Random pool** mode (`generateOneRandomLevel`): by default the board is inferred after sampling `templateParams` (including **`heart`**). Set `useLegacyPoolGrid: true` in `pool.paramRanges` to restore sampling `gridDimensions` first. `poolInferSeed` is set from `config.seed` by the CLI for stable inference per slot.
    ...
    - `layering`:
      ...
      - `layerShapeOptions`: optional strategy params:
        ...
        - for `paramSweep`: `{ sweep: 'thickness' | 'footprintZ' | 'radius', … }` — **`thickness`**: merged `templateParams` per layer (see §5.1); **`footprintZ`**: same `templateParams` every layer, **`z = minZ + layerIndex`** for z-aware footprints; **`radius`**: symmetric **radius** steps from **`maxRadius`** (bottom) down to **`minRadius`** (top), each layer **`getTemplateCells(..., { z: minZ + layerIndex })`** — pyramid silhouette + half-cell consistency (e.g. `diamond`).
...
- **Templates**: `tools/levelgen/templates.js`
  - `getTemplateCells(templateId, templateParams, gridWidth, gridHeight, options?)` returns an array of allowed `{ x, y }` cells that define the 2D silhouette. Optional **`options.z`** (default **`0`**) is the **absolute layer index** used with [`tile-layering.js`](tile-layering.js) (`tileCenterInCells`): odd `z` applies the half-cell diagonal offset, so the **integer cell mask** can differ from even `z` at the silhouette edge. Internally, each template first builds a legacy integer mask, then expands it so every included `(x,y)` has a **tile footprint** (center from `tileCenterInCells`, extent ±0.5 in cell x/y) intersecting that legacy region—matching how overlap is reasoned about in the game. Changing `z` or regenerating levels can **change** layouts; re-run `npm run build` / `node tools/generate-levels.js` when template geometry updates.
...
- **Generator**: `tools/levelgen/generator.js`
  ...
  - `generateOneLevel(rng, batch, levelId, inferCtx?)`: resolves `gridWidth`/`gridHeight` via `resolveBatchGrid` (explicit sizes, legacy `gridSize`, or **`inferGridDimensions`** when both dimensions are omitted). Pass **`inferCtx`** `{ seed, batchIndex, slotIndex }` whenever the batch omits the grid (CLI and designer use the same indices as `resolveBatchVariation`).
...
- **Interactive designer** (local only): `npm run leveldesign` starts `tools/leveldesign-server.js` (default [http://127.0.0.1:8765](http://127.0.0.1:8765)), which also serves `/style.css`, `/assets/`, `/tile-layering.js`, and `/lib/*.js` from the repo root. The full-board preview imports **`lib/board-view.js`** and **`lib/tile-types.js`** (same modules as `game.js`) so layout math and emoji mapping are not duplicated. Edit generator parameters in the browser, see the **template footprint** (compact), **top tile per column** (quick view), a **full-level board** matching the game (**wood tile**, **emoji** from `TILE_TYPES`, **`boardTileCenterPx`** / odd-`z` half-cell offset as in `game.js`), **per-layer silhouettes** (each depth’s allowed `(x,y)` in the generated level’s `layerSilhouettes`), and **solver / difficulty metrics** from `scoreLevel`. Uses the same `batch` shape as `tools/levelgen/config.js`. **Fixed board size (explicit grid)** is off by default: the designer omits `gridWidth`/`gridHeight` from preview requests so the server infers the board like batch mode; after preview, the width/height fields show the resolved size (inputs stay disabled until you enable fixed size). Loading a batch from **config.js** that already defines a grid (or `gridSize`) checks the box and keeps those dimensions. **Config seed** + **level id** match `generateLevelsFromConfig`’s per-level RNG (first level in a run = id `1`). Override with **level seed** for a direct `mulberry32` seed. Set `LEVELDESIGN_PORT` to change the port. `layerSilhouettes` is computed during generation but **stripped** from `levels.generated.js` so the runtime bundle stays small.
...
#### 3.0. Total triplets from shape (`zipf` and `weightedTriplets`)

For `zipf` and `weightedTriplets`, **`totalTriplets` is always derived** from layered silhouette capacity (you do not set it in the batch or designer):
...
3. Sets `totalTriplets = floor(maxTiles * templateTripletFillRatio / 3)` (default `templateTripletFillRatio` = 1), then ensures `3 * totalTriplets ≤ maxTiles`.

Optional batch / `pool.paramRanges` field: `templateTripletFillRatio` (0–1).

Silhouettes are built before shuffling tile placement, so RNG consumption stays tied to that order for a given seed.
...
- Fields:
  ...
  - Total triplet count comes from shape capacity (§3.0).
- Behavior:
...
// totalTriplets is derived from layered shape (§3.0)
```
...
- **`layerShape`** controls each layer’s allowed cells (and thus its fill shape):
  ...
  - **`'paramSweep'`**: Each layer’s silhouette is produced by **`getTemplateCells`** with **`z = minZ + layerIndex`** — not by editing a single base footprint. **`layerShapeOptions.sweep`**:
    - **`'footprintZ'`**: Same `templateParams` on every layer; only the footprint **`z`** changes so odd layers get the correct half-cell expansion (works for **any** `templateId`, e.g. **`diamond`**). Equivalent silhouette semantics to **`layerShape: 'full'`**, but routed through the paramSweep code path.
    - **`'radius'`**: **Symmetric radius pyramid** — bottom layer uses **`maxRadius`**, top uses **`minRadius`**, with **integer radii** stepped down (`max`, `max−1`, …, `min`) and layers spread **evenly across that ladder** (same index mapping as thickness sweep). Requires **`templateParams.radius`** or **`layerShapeOptions.maxRadius`**; **`minRadius`** defaults to **1**. Supported `templateId` values: **`diamond`**, **`circle`**, **`triangle`**, **`hexagon`**, **`cross`**, **`ring`**, **`t`**, **`u`**, **`heart`**, **`spiral`**, **`letter`**. If **`radiusX` / `radiusY`** are set, `resolveRadii` may ignore **`radius`** — prefer symmetric **`radius`** for this sweep.
    - **`'thickness'`**: **Bottom** layer (index 0) uses **`maxThickness`**, **top** uses **`minThickness`**. Thickness values run along the **integer ladder** from max down to min (`max`, `max−1`, …, `min`), with layers spread **evenly across that ladder** (by index). Defaults: **`minThickness`** = **1**; **`maxThickness`** = batch `templateParams.thickness` if set, else a template default (e.g. **2** for `cross`, **1** for `heart` / `spiral`). **`maxThickness`** may be set explicitly in `layerShapeOptions`. Supported `templateId` values for thickness sweep: **`cross`**, **`ring`**, **`t`**, **`u`**, **`heart`**, **`spiral`**, **`letter`**. For **`cross`**, sweeping **thickness** thins the bars but does **not** shorten arm length; use smaller **`radius`** in `templateParams` or **`sweep: 'radius'`** if you need shorter arms on upper layers.
