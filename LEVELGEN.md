...
- **`layerShape`** controls each layer’s allowed cells (and thus its fill shape):
  ...
  - **`'paramSweep'`**: Each layer’s silhouette is produced by **`getTemplateCells`** with **merged** `templateParams` and **`z = minZ + layerIndex`** — not by editing a single base footprint. **`layerShapeOptions.sweep`** must be **`'thickness'`** (only option today). **Bottom** layer (lowest `z`, index 0) uses **`maxThickness`**, **top** layer uses **`minThickness`**. Thickness values are chosen along the **integer ladder** from max down to min (`max`, `max−1`, …, `min`), with layers spread **evenly across that ladder** (by index), so middle integers are not skipped the way continuous lerp + `Math.round` can skip (e.g. 4→1 in three layers). Defaults: **`minThickness`** = **1**; **`maxThickness`** = batch `templateParams.thickness` if set, else a template default (e.g. **2** for `cross`, **1** for `heart` / `spiral`). **`maxThickness`** may be set explicitly in `layerShapeOptions`. Supported `templateId` values for thickness sweep: **`cross`**, **`ring`**, **`t`**, **`u`**, **`heart`**, **`spiral`**, **`letter`**. For **`cross`**, sweeping **thickness** thins the bars but does **not** shorten arm length; use smaller `radius` in `templateParams` or a future radius sweep if you need a tiny plus on top.
