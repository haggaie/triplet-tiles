  ...
  - `generateLevelsFromConfig(config)`:
    - For each batch:
      ...
      - Converts the sequence into a multi-layer layout over the template silhouette:
        ...
        - **Layer silhouettes**: each layer’s allowed cells come from `layerShape`: `'full'` = same as base; `'pyramid'` = base shrunk each layer by dropping cells with too few in-silhouette cardinal neighbors (see `pyramidMinNeighbors`, default 2); `'paramSweep'` = per-layer `getTemplateCells` with swept params (e.g. `sweep: 'thickness'`); `'shift'` = base shifted by `(shiftDx, shiftDy)` per layer index; `'randomErosion'` = connected edge-erosion per layer with optional small random shifts.
