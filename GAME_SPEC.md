...
- **Problem**: Board size is capped to fit the viewport (separate width/height budgets from `#board-scroll`). **Narrow viewports (under 768px)** use a full-bleed mat with **no 448px width cap** (wide-but-narrow devices aren’t stuck at 448px with side gaps); **wider (tablet/desktop, 768px and up)** use the inset shell, `min(448px, 92vw)`-style width cap, and 12px padding on `.board-scroll-align` as before. Cell size is shared for both axes: `widthPx = effectiveCell × (gridWidth + 2)` and `heightPx = effectiveCell × (gridHeight + 2)`, so large grids or a high `--board-cell-min` can still force panning inside `#board-scroll`.
- **Approach**:
  1. **Floor** — CSS token `--board-cell-min` (default `40px`): effective cell size is `max(fitCell, boardCellMin)` so tiles stay legible. On **narrow viewports (under 768px)**, cell size is derived from the **width** budget first so the mat spans the scrollport width; extra height scrolls inside `#board-scroll`. Wider layouts keep the classic “fit inside width and height” cell size.
