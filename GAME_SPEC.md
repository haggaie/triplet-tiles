...
- **Problem**: Board size is capped to fit the viewport (separate width/height budgets from `#board-scroll`). **Narrow viewports (under 768px)** use a full-bleed mat with **no 448px width cap** (wide-but-narrow devices aren’t stuck at 448px with side gaps); **wider (tablet/desktop, 768px and up)** use the inset shell, `min(448px, 92vw)`-style width cap, and 12px padding on `.board-scroll-align` as before. Cell size is shared for both axes: `widthPx = effectiveCell × (gridWidth + horizontalMarginCells)` and `heightPx = effectiveCell × (gridHeight + verticalMarginCells)` (defaults **2** each on wide; narrow uses a slightly smaller **horizontal** margin sum so tiles sit closer to the wood edge), so large grids or a high `--board-cell-min` can still force panning inside `#board-scroll`.
- **Approach**:
  1. **Floor** — CSS token `--board-cell-min` (default `40px`): effective cell size is `max(fitCell, boardCellMin)` so tiles stay legible. On **narrow viewports (under 768px)**, cell size is derived from the **width** budget first so the mat spans the scrollport width; extra height scrolls inside `#board-scroll`. Wider layouts keep the classic “fit inside width and height” cell size. **Horizontal margin** (space from the outermost tiles toward the board edge) uses a slightly smaller width divisor on narrow viewports than on wide (see `BOARD_MARGIN_CELLS_H_NARROW` in `game.js`).
  2. **Pan** — The playfield (`#board`) is a rectangle (often square when `gridWidth === gridHeight`) sized by those formulas. When it exceeds the visible scrollport (`#board-scroll`), the player scrolls horizontally and/or vertically inside that region. The document body should not gain horizontal scroll from the board. **`#board-scroll` must not use flex centering on itself** (it breaks max `scrollLeft` / `scrollTop` in some engines); centering is done on an inner `.board-scroll-align` (`width: max-content; margin-inline: auto`) with padding for shadows (12px on wide layouts; vertical-only on narrow).
