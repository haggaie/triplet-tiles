...
- **Tab order**: Header (Levels, Restart) → **`#locale-select`** → **`#fullscreen-toggle`** (when supported) → **master mute** (`#audio-master-mute-toggle`) → **`#settings-open-button`** → power-up buttons → **`#board`** (one tab stop when not in Remove Type mode) → **`#tray`** only after activating **Remove Type** with tiles in the tray (then `#board` is `tabindex="-1"` until the mode ends). Empty slots are skipped. (Score is not focusable.) **Install app** and granular **music / SFX** (and **vibrate**, when `navigator.vibrate` exists) live in **Settings** (`#settings-overlay`).
