...
- **Tab order**: Header (Levels, Restart) → **`#locale-select`** → fullscreen / install (when visible) → music / SFX / haptics controls (buttons and range inputs) → power-up buttons → **`#board`** (one tab stop when not in Remove Type mode) → **`#tray`** only after activating **Remove Type** with tiles in the tray (then `#board` is `tabindex="-1"` until the mode ends). Empty slots are skipped. (Score is not focusable.)
- **i18n:** Strings load from `lib/i18n.js` (English and Spanish). Playwright defaults to browser locale; use `window.__tripletTestHooks.setLocaleForTest('en')` if a test must assert English copy. Optional URL override: `?lang=en` / `?lang=es` (writes preference to `localStorage`).
