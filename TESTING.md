...
- **Render efficiency (Playwright):** `tests/render-efficiency.spec.js` — validates that the board renders only the minimum necessary DOM nodes and minimises style/attribute mutations per interaction. Six tests: (1) `#board [data-tile-id]` count equals non-removed minus occluded at all times; (2) occluded tiles (same `(x,y)` and z-parity, hidden under a higher-z tile) have no DOM node and appear after their cover is picked; (3) a single pick produces at most 1 child removal + 1 child addition, with style mutations O(dirty tiles); (4) `getRenderStats()` after a pick reports `isIncremental=true` and `synced` is a small fraction of DOM tile count; (5) a second full render skips `>50%` of tiles when nothing changed; (6) the heavy 546-tile level has `>30%` tiles occluded and DOM count well below total non-removed.
...
**Render-efficiency hooks** added alongside the performance hooks: `getRenderStats()` returns `{ synced, skipped, created, removed, isIncremental, boardDomTileCount }` from the last `renderBoard` call; `getOccludedTileCount()` returns the number of tiles excluded from the DOM due to same-position occlusion; `getOccludedTileIds()` returns those IDs as an array; `triggerFullRenderForTest()` calls `renderBoard(false, {})` without modifying game state (useful to test the skip-unchanged optimisation on a second pass).
