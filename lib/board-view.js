...
/**
 * Playfield size and cell size from grid dimensions (width/height each add margin cells to grid size).
 * Single cellSize so tiles stay square; board may be rectangular if gridWidth !== gridHeight.
 *
 * @param {number} gridWidth
 * @param {number} gridHeight
 * @param {{ maxW?: number, maxH?: number, cellMinPx?: number, preferWidthFill?: boolean, horizontalMarginCells?: number, verticalMarginCells?: number }} [options]
 * @param [options.horizontalMarginCells] Extra columns beyond `gridWidth` (default 2 → one margin cell per side). Lower = tighter to board edge on phones.
 * @param [options.verticalMarginCells] Extra rows beyond `gridHeight` (default 2).
...
export function measureBoardLayoutFromFit(gridWidth, gridHeight, options = {}) {
  ...
  const mW = options.horizontalMarginCells ?? 2;
  const mH = options.verticalMarginCells ?? 2;
  const cellBaseW = maxW / (gw + mW);
  const cellBaseH = maxH / (gh + mH);
  ...
  const widthPx = cellSize * (gw + mW);
  const heightPx = cellSize * (gh + mH);
