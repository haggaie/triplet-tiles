...
/**
 * Types with count ≥3 at this round, in **first left-to-right appearance** in the tray
 * (each distinct type considered once when first seen).
 */
export function getTripleRemovalTypeOrder(trayTiles, counts) {
  const seen = new Set();
  const order = [];
  for (const t of trayTiles) {
    ...
    seen.add(ty);
    if ((counts.get(ty) || 0) >= 3) order.push(ty);
  }
  return order;
}
 ...
 * order (matches `handleMatchingInTray` in game.js). When several types qualify, process
 * in **first-appearance order** along the tray (`getTripleRemovalTypeOrder`).
 *
 * @returns {{ trayTiles: Array<{id: string, type: (number|string)}>, scoreDelta: number, removedTypes: Array<number|string> }}
 */
export function removeMatchingTriplesOneRound(trayTiles) {
  ...
  const toRemoveTypes = getTripleRemovalTypeOrder(trayTiles, counts);
 ...
 * @param {Array<number|string>} typesInOrder
 */
 ...
 * @param {Array<{id: string, type: (number|string)}>} committedTrayTiles
 * @param {{ id: string, type: (number|string) } | null | undefined} flyingTileRef
 * @returns {{ trayTiles: Array<{id: string, type: (number|string)}>, length: number }}
 */
