...
export function createAudioService({ storageKey, musicUrl, sfxUrlMap }) {
   ...
   * @param {{ traySlot?: number }} [opts]
   */
  async function playSfxAsync(eventId, opts = {}) {
    ...
    src.buffer = buffer;
    let rate = 1 + (Math.random() * 0.06 - 0.03);
    if (opts.traySlot != null && Number.isFinite(opts.traySlot)) {
      rate *= 1 + opts.traySlot * 0.015;
    }
   ...
   * @param {{ traySlot?: number }} [opts]
   */
  function playSfx(eventId, opts) {
    void playSfxAsync(eventId, opts);
  }
