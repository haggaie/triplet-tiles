...
export function createAudioService({ storageKey, musicUrl, sfxUrlMap }) {
  ...
  function prepareSfx() {
    ...
    sfxLoadPromise = (async () => {
      try {
        ...
        /* Do not call resume() here: this IIFE runs after the user-gesture stack ends;
         * Chrome warns and may ignore it. unlock() already invokes resume() on pointerdown. */
  ...
  /** Mute or unmute both music and SFX in one persisted update. */
  ...
  if (typeof document !== 'undefined') {
    ...
    /**
     * `AudioContext.resume()` must run in a user-gesture stack; the first `unlock()` call
     * sometimes loses the race (or `unlockAudioForTest()` runs from the console). Any later
     * tap should move the context to `running` so SFX decode/play can proceed.
     */
    document.addEventListener(
      'pointerdown',
      () => {
        if (!unlocked || !sfxContext || sfxContext.state !== 'suspended') return;
        void sfxContext.resume();
      },
      { capture: true }
    );
  ...
  /** Reset music, SFX, and haptics to defaults and persist (mirrors fresh install for audio key). */
  function resetToDefaults() {
    ...
    applySfxBusGain();
    const el = ensureMusic();
    if (state.musicMuted || state.musicVolume <= 0) el.pause();
    else playIfAllowed();
  }

  load();

  function resumeSfxIfSuspended() {
  ...
  return {
    ...
    getDiagnostics() {
      ...
      return {
        unlocked,
        sfxBuffersLoaded,
        sfxLoadPending: unlocked && sfxLoadPromise != null && !sfxBuffersLoaded,
        ...
        musicVolume: state.musicVolume,
        musicUrl: musicUrl,
        musicCurrentSrc: el.currentSrc || null,
        musicReadyState: el.readyState,
        musicNetworkState: el.networkState,
        musicPaused: el.paused,
        /** MediaError code: 1=aborted, 2=network, 3=decode, 4=src not supported; null if none. */
        musicErrorCode: err ? err.code : null,
        musicDuckMult,
        hapticsEnabled: state.hapticsEnabled,
        vibrateSupported:
