...
- [x] **Audio & haptics** — Background music (optional mute), sound effects for pick/match/tray/loss/win, and **vibration** on supported devices (respect system "reduce motion" / accessibility preferences where applicable). **Done:** ambient loop, SFX pipeline (Web Audio, duck, preload), music + SFX + haptics UI (mute / volume / vibration toggle), `prefers-reduced-motion` default, rate-limit, persistence. Browsers without `navigator.vibrate` (e.g. Firefox Android) show a disabled toggle with explanatory copy. SFX `AudioContext` unlock on capture-phase `pointerdown` for reliable autoplay. Spec: [AUDIO_DESIGN.md](AUDIO_DESIGN.md); full phase log: [AUDIO_IMPLEMENTATION_PLAN.md](AUDIO_IMPLEMENTATION_PLAN.md).
...
- Rework the chrome to be smaller - perhaps we need a settings dialog instead of the multiple volume slides.
...
- [ ] **Internationalization (i18n)** — Extract user-visible strings; locale selection; RTL/layout considerations for future locales; date/number formatting if stats become localized.
